...
@Lazy
@Component("6.3")
public class CBS63Client implements CBSClient {

	private static final Logger logger = LoggerFactory.getLogger(CBS63Client.class);
	...
	public CBS63Client(@Autowired AppConfig appConfig) {
	...
	public Mono<NameEnquiryResponse> nameEnquiry(NameEnquiryRequest nameEnquiryRequest) {
		try {
			...
			return Mono.just(new NameEnquiryResponse(cbsToNpciResponseMapper.map(nameEnquiryResponse.responseCode), nameEnquiryResponse.responseMessage,  
					nameEnquiryResponse.payeeName));
		} catch (Exception e) {
			logger.error("error processing credit at CBS.", e);
		}
		return null;
