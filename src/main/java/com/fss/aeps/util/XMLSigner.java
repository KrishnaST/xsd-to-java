...
public class XMLSigner {
	...
	public final Document generateXMLDigitalSignature(final Document document) {
		...
		try {
			xmlSignature = XMLUtils.getXMLSignature();
	...
	/*	public static void main(String[] args) throws TransformerException, SAXException, IOException, ParserConfigurationException, Exception {
		XMLSigner signer = new XMLSigner("ini/signer.pfx", "signer123", "upi");
		String xml = "<test><sub>hi</sub></test>";
		final Document document = signer.generateXMLDigitalSignature(XMLUtils.stringToDocument(xml));
		final boolean b = signer.isValidDigitalSignature(XMLUtils.documentToString(document));
