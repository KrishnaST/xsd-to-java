...
public final class ISO8583NameEnquiry implements AcquirerTransaction<RespValAdd> {
	...
	public ISO8583NameEnquiry(Socket socket, ISO8583Message isoRequest) {
		this.socket = socket;
	...
	public final void run() {
		try {
			...
			final ReqValAdd request = new ReqValAdd();
			...
			txn.setTs(new Date());
			txn.setType(PayConstant.NAME_ENQ);
			...
			payer.getDevice().getTag().add(new Tag(DeviceTagNameType.TYPE, tlv.get("002"))); //APK
			...
			AcquirerTransactionMap.nameEnquiryTransactions.put(request.getTxn().getType().name()+request.getTxn().getId(), this);
			final Mono<Ack> mono = npciWebClient.nameEnquiry(request);
			mono.subscribe(ack -> {
				//todo
	...
	@Override
	public void processResponse(RespValAdd response) {
		try(OutputStream os = socket.getOutputStream()) {
			...
			if(response.getResp().getResult() == ResultType.SUCCESS) {
				...
				isoResponse.put(43, response.getResp().getMaskName().trim());
			...
			logger.error("error occured sending response back to acquirer", e);
		}
	}
