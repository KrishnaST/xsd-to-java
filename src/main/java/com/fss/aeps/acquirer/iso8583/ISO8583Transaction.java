...
public final class ISO8583Transaction implements AcquirerTransaction<RespPay> {
	...
	public ISO8583Transaction(Socket socket, ISO8583Message isoRequest) {
		this.socket = socket;
	...
	public final void run() {
		try {
			...
			payerCred.setType(CredType.PRE_APPROVED);
			payerCred.setSubType(CredSubType.NA);
			payerCred.setData(new Data("MDB8QVBQUk9WRUQ"));
			...
			payeeAccount.getDetail().add(new Detail(AccountDetailType.ACTYPE, "SAVINGS"));
			payeeAccount.getDetail().add(new Detail(AccountDetailType.ACNUM, tlv.get("062")));//test P2A Trans
			payeeAccount.getDetail().add(new Detail(AccountDetailType.IFSC, tlv.get("059")));
			payee.setAmount(amount);
			
			transactionService.registerTransaction(request);
			
			AcquirerTransactionMap.paymentTransactions.put(request.getTxn().getType().name()+request.getTxn().getId(), this);
			final Mono<Ack> mono = npciWebClient.payment(request);
			mono.subscribe(ack -> {
				//todo
	...
	public void processResponse(RespPay response) {
		try(OutputStream os = socket.getOutputStream()) {
			final Optional<ImpsTransaction> transaction = transactionService.findById(response.getTxn().getId(), response.getTxn().getType());
	...
	public static void main(String[] args) {
		String upiInstActSettlement="N";
		String upisetAct = upiInstActSettlement == "C" ? "Y"+upiInstActSettlement.equals("N") : "";
		System.out.println("---upisetAct----"+upisetAct);
		if(upisetAct.equalsIgnoreCase("Yfalse")) {
			System.out.println("---upisetAct----First---- "+upisetAct);
			System.out.println("Y");
		}else {
			System.out.println("---upisetAct----Second---- "+upisetAct);
			System.out.println("");
		}
	}
}